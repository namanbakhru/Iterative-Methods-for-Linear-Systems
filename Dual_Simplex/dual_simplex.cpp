...
int main(){
    int m, n, i, j, number_iterations=0, q, p, *basic_ind = new int[m], *non_basic_ind = new int[n];
    bool opt_flag = false;
    cout<<"Enter the values of m and n:";
    cin>>m>>n;
    double **array = new double*[m], *b = new double[m], *delta = new double[m+n], *c = new double[n+m], temp_cost, min=1, temp=0;
    array = array - 1;
    b = b - 1;
    delta = delta - 1;
    c = c - 1;
    ...
    for(i=1; i<=m; i++)
        cin>>b[i];
    cout<<"Enter the n cost coefficients: ";
    for(i=1; i<=n; i++)
        cin>>c[i];
    for(;i<=n+m; i++)
        c[i] = 0;
    ...
    while(true){
        ...
        if(p == -1){
            cout<<"The LPP has feasible solution"<<endl;
            break;
        }
        ...
        for(i=1; i<=n; i++){
            ...
            if(array[p][non_basic_ind[i]]<0){
                if(delta[non_basic_ind[i]]/array[p][non_basic_ind[i]]>min){
                    min = delta[non_basic_ind[i]]/array[p][non_basic_ind[i]];
                    q = i;
                }
            }
        }
